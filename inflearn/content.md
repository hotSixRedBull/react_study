...
- 리액트 16.8에 추가됨.
  - 이전에는 클래스형 컴포넌트를 사용.
  - 이젠 클래스형 컴포넌트를 훅이 대체하는 상황.
    - https://mk-v1.kakaocdn.net/dn/if-kakao/conf2019/conf_video_2019/1_104_01_m1.mp4
- `useState`: 상태값 추가
  - 상태값 변경은 비동기, batch로 처리한다.
    - 비동기이기 때문에 두번 연속호출하면 변경보다 전달이 빠를 수 있다.
    - batch이기 때문에 여러 작업을 묶어서 한 번에 처리한다.
  - 동기로 처리하면 성능 이슈가 발생하기 때문.
- `useEffect`: 부수효과 처리
  - 모든 부수효과를 처리하는 게 좋다.
  - 첫 번째 매개변수(`부수효과 함수`): 함수, 컴포넌트가 렌더링 결과가 실제 돔에 전달된 후 비동기적으로 실행한다.
    - 부수효과 함수의 반환값: 다음 부수 효과 함수가 호출되기 직전 혹은 컴포넌트가 언마운트 되기 직전에 호출된다.
      - 이벤트 핸들러를 등록하고, 해제하는 경우 해제하는 함수를 반환하면서, 의존성 배열을 비우는 패턴이 있다.
      ```
      useEffect(() => {
        window.addEventHandler('resize', onResize);
        return () => {
          window.removeEventHandler('resize', onResize);
        };
      }, []);
      ```
  - 두 번째 매개변수(`의존성 배열`): 이 배열에 있는 값들 중 하나가 변경될 때만 부수효과가 실행된다.
    - 빈 배열이 들어간다면, 한번만 렌더링된다.
    - 컴포넌트의 상태, 속성값, 내부에서 정의된 지역변수/함수는 의존성 배열에 작성해줘야 한다.
      - 예외: useState의 상태값 변경함수는 불변이 보장된다.
    - 내부 함수의 경우, 컴포넌트가 렌더링 될때마다 새로 생성이 되니 `useCallBack`을 사용하는 것을 고려해야 한다.
- `useReducer`: 여러 상태값을 관리
- 커스텀 훅
  - 재사용성을 높일 수 있다.
  - `use...`형태를 띄는 것을 권장

### Rules to follow on hooks

- 하나의 컴포넌트에서 훅을 호출하는 순서는 항상 같아야 한다.
  - 리액트는 호출 순서로 훅을 관리한다.
- 훅은 함수형 컴포넌트 또는 커스텀 훅 안에서만 호출되어야 한다.
  - 클래스형 컴포넌트나 일반형 함수에서 사용할 수 없다.
  - if, for, 함수 안에서 사용하지 말 것.
  - 자주하는 실수
    - 반환문 뒤에 훅을 호출하는 경우
...
- useContext
- useRef
  - useState보다 useRef가 적합한 경우: UI와 상관없는 데이터를 처리하고 싶을 때.
    - useState의 경우 렌더링을 다시 시키기 때문에, useRef가 더 적합할 수 있다.
- useMemo
  - 메모이제이션 기능 제공
  - 계산량이 많은 함수의 반환 값을 재활용
  - `useMemo(메모함수, 의존성 배열)`: 의존성 배열의 요소가 변경되면 메모함수를 실행한다.
- useCallback
  - 메모이제이션 기능 제공
- useReducer
- useImperativeHandle
